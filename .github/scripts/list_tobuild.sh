      ...
      if [ ! -s tobuild.txt ] && [ -f "packages.json" ]; then
            ...
            if [ $counter -gt 4 ]; then
                ...
                if [ -s /tmp/resetpkgs ]; then
                    ...
                    cat /tmp/resetpkgs | xargs -i bash -c 'pkgcounter=0; retrypath="logs/$(cat runstarttime)/retries/{}"; if [ -f "$retrypath" ]; then pkgcounter=$(<$retrypath); fi; pkgcounter=$((pkgcounter+1)); mkdir -p $(dirname $retrypath); echo $pkgcounter > "$retrypath"; if [ $pkgcounter -lt 3 ]; then rm {} && rm "logs/$(cat runstarttime)/retries/{}"; fi'
                    git add lists
                    git add logs
                fi
            fi
